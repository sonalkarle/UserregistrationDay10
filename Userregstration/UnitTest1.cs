...
public class Tests
{
    ...
    //TC5: Validate the password
    public void Givenpassword_WhenValid_ShouldReturnTrue()
    {
        bool result = useregex.ValidateMobileNumber("Adv@q12");
        Assert.IsTrue(result);
    }
    //TC5.1: Password should not be null
    public void GivenPassword_WhenNull_ShouldThrowCustomException()
    {
        try
        {
            bool result = useregex.ValidatePassword(null);
        }
        catch (UserRegistrationException exception)
        {
            Assert.AreEqual(UserRegistrationException.ExceptionType.ENTERED_NULL, exception.exceptionType);
    ...
    //TC5.2: Password should not be empty
    [Test]
    public void GivenPassword_WhenEmpty_ShouldThrowCustomException()
    {
        try
        {
            bool result = useregex.ValidatePassword("");
        }
        catch (UserRegistrationException exception)
        {
            Assert.AreEqual(UserRegistrationException.ExceptionType.ENTERED_EMPTY, exception.exceptionType);
    ...
    //TC5.3: Password should maintain proper length
    [Test]
    public void GivenPassword_WhenLessThanMinimumLength_ShouldThrowCustomException()
    {
        try
        {
            bool result = useregex.ValidatePassword("Gha$%");
        }
        catch (UserRegistrationException exception)
        {
            Assert.AreEqual(UserRegistrationException.ExceptionType.ENTERED_MINIMUM_LENGTH, exception.exceptionType);
